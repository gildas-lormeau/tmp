...
const WINDOWS_PROGRAM_FILES_PREFIXES = ["C:\\Program Files\\", "C:\\Program Files (x86)\\"];
const WINDOWS_LOCAL_APP_DATA_VARIABLE = "%LOCALAPPDATA%";
const MACOS_APPLICATIONS_PREFIX = "/Applications/";

const { build, env, readDir, copyFile, mkdir, Command, errors, remove, stat, lstat } = Deno;
...
const { join } = path;
export { getExecutablePaths, findExecutablePath, spawnBrowser, getDebugPort, copyProfile, pruneProfile, removeProfileLockEntries };

function getExecutablePaths(paths, os = build.os, variables = env) {
	const listedPaths = paths[os] || [];
	const systemPaths = listedPaths.filter(executablePath => !executablePath.startsWith(WINDOWS_LOCAL_APP_DATA_VARIABLE));
	let userPaths = [];
	if (os == "windows") {
		const localAppDataPath = (variables.get("LOCALAPPDATA") || "").replace(/\\+$/, "");
		if (localAppDataPath) {
			userPaths = listedPaths.map(executablePath => {
				if (executablePath.startsWith(WINDOWS_LOCAL_APP_DATA_VARIABLE)) {
					return localAppDataPath + executablePath.substring(WINDOWS_LOCAL_APP_DATA_VARIABLE.length);
				}
				const prefix = WINDOWS_PROGRAM_FILES_PREFIXES.find(prefix => executablePath.startsWith(prefix));
				if (prefix) {
					return localAppDataPath + "\\" + executablePath.substring(prefix.length);
				}
			});
		}
	} else if (os == "darwin") {
		const homePath = (variables.get("HOME") || "").replace(/\/+$/, "");
		if (homePath) {
			userPaths = listedPaths
				.filter(executablePath => executablePath.startsWith(MACOS_APPLICATIONS_PREFIX))
				.map(executablePath => homePath + executablePath);
		}
	} else {
		const directories = (variables.get("PATH") || "").split(":").filter(directory => directory.startsWith("/"));
		const names = listedPaths.map(executablePath => executablePath.substring(executablePath.lastIndexOf("/") + 1));
		userPaths = Array.from(new Set(names)).flatMap(name => directories.map(directory => directory.replace(/\/+$/, "") + "/" + name));
	}
	return Array.from(new Set(systemPaths.concat(userPaths.filter(Boolean))));
}

async function findExecutablePath(executablePaths, os = build.os) {
	for (const executablePath of executablePaths) {
		if (await executableExists(executablePath, os)) {
			return executablePath;
		}
	}
}

async function executableExists(executablePath, os) {
	try {
		await stat(executablePath);
		return true;
	} catch {
		if (os == "windows") {
			return lstat(executablePath).then(() => true, () => false);
		}
		return false;
	}
}
...
async function spawnBrowser(executablePath, args, temporaryProfilePath) {
	if (build.os == "windows") {
		return new Command(executablePath, { args, stdout: "inherit", stderr: "inherit" }).spawn();
	}
	try {
		await stat(executablePath);
	} catch {

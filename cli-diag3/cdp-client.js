...
async function getPageData(options) {
	...
	try {
		...
		console.error("DIAG before: targetInfo = await CDP.createTarget(EMPTY_PAGE_URL); at " + Date.now());
		targetInfo = await CDP.createTarget(EMPTY_PAGE_URL);
		console.error("DIAG before: cdp = new CDP(targetInfo); at " + Date.now());
		cdp = new CDP(targetInfo);
		console.error("DIAG before: await setupConsoleLogging(cdp, pageContext); at " + Date.now());
		await setupConsoleLogging(cdp, pageContext);
		console.error("DIAG before: await setupBrowserWindow(cdp, targetInfo.id, pageContext); at " + Date.now());
		await setupBrowserWindow(cdp, targetInfo.id, pageContext);
		console.error("DIAG before: await setupSecurity(cdp, pageContext); at " + Date.now());
		await setupSecurity(cdp, pageContext);
		console.error("DIAG before: await setupDeviceEmulation(cdp, pageContext); at " + Date.now());
		await setupDeviceEmulation(cdp, pageContext);
		console.error("DIAG before: await setupNetwork(cdp, pageContext); at " + Date.now());
		await setupNetwork(cdp, pageContext);
		console.error("DIAG before: await setupDownloadBehavior(cdp); at " + Date.now());
		await setupDownloadBehavior(cdp);
		console.error("DIAG before: await setupScriptInjection(cdp, pageContext); at " + Date.now());
		await setupScriptInjection(cdp, pageContext);
		console.error("DIAG before: await setupDialogHandling(cdp, pageContext); at " + Date.now());
		await setupDialogHandling(cdp, pageContext);
		console.error("DIAG before: const contextId = await getContextId(cdp, pageContext); at " + Date.now());
		const contextId = await getContextId(cdp, pageContext);
		console.error("DIAG before: const pageDataPromise = setupPageDataCapture(cdp, pageContext); at " + Date.now());
		const pageDataPromise = setupPageDataCapture(cdp, pageContext);
		console.error("DIAG before: await setupBindings(cdp, pageContext); at " + Date.now());
		await setupBindings(cdp, pageContext);
		console.error("DIAG before: await capturePageData(cdp, contextId, pageContext); at " + Date.now());
		await capturePageData(cdp, contextId, pageContext);
		console.error("DIAG before: await disableCdpDomains(cdp, pageContext); at " + Date.now());
...
async function setupDeviceEmulation({ Browser, Emulation, Runtime }, { options, debugMessages, browserInfo }) {
	...
	if (needsDeviceMetrics) {
		console.error("DIAG before: await setupDeviceMetrics({ Emulation, Runtime }, { options, debugMessages }); at " + Date.now());
		await setupDeviceMetrics({ Emulation, Runtime }, { options, debugMessages });
	}
	if (options.emulateMediaFeatures) {
		console.error("DIAG before: await setupMediaFeatures({ Emulation }, { options, debugMessages }); at " + Date.now());
...
async function setupNetwork({ Fetch, Network }, { options, debugMessages, httpInfo, documentInfo, blockedURLPatterns }) {
	...
	if (options.httpHeaders && !hasUnsafeHttpHeaders(options)) {
		console.error("DIAG before: await setupHttpHeaders({ Network }, { options, debugMessages }); at " + Date.now());
		await setupHttpHeaders({ Network }, { options, debugMessages });
	}
	if (options.browserCookies && options.browserCookies.length) {
		console.error("DIAG before: await setupCookies({ Network }, { options, debugMessages }); at " + Date.now());
...
async function setupNetworkSession({ Fetch, Network }, sessionId, { options, debugMessages }) {
	...
	if (options.httpHeaders && !hasUnsafeHttpHeaders(options)) {
		console.error("DIAG before: await setupHttpHeaders({ Network }, { options, debugMessages }, sessionId); at " + Date.now());
...
async function setupFrameSession({ Browser, Emulation, Fetch, Network, Security }, sessionId, { options, debugMessages }) {
	...
	if (options.emulateMediaFeatures) {
		console.error("DIAG before: await setupMediaFeatures({ Emulation }, { options, debugMessages }, sessionId); at " + Date.now());
...
async function setupBindings({ Page, Runtime }, { options, debugMessages, documentInfo, browserInfo, blockedURLPatterns, fetchAbortController }) {
	...
	if (options.embedScreenshot && options.compressContent) {
		console.error("DIAG before: await setupScreenshotCapture({ Page, Runtime }, { options, debugMessages }); at " + Date.now());
		await setupScreenshotCapture({ Page, Runtime }, { options, debugMessages });
	}
	if (options.embedPdf && options.compressContent) {
		console.error("DIAG before: await setupPdfCapture({ Page, Runtime }, { options, debugMessages }); at " + Date.now());

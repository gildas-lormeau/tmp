...
function setupHttpInfoCapture(pageContext) {
	...
	const { options, context, httpInfo, documentInfo } = pageContext;
	...
	listen(pageContext, "network.responseStarted", params => {
		...
		const isDocument = params.context === context && navigation &&
		...
		if (isDocument) {
			if (REDIRECT_STATUS_CODES.includes(response.status)) {
				...
				const responseHeaders = response.headers.map(header => ({ name: header.name, value: header.value.value }));
				if (!documentInfo.url) {
...
async function handleFetchRequest(params, pageContext) {
	...
	const { options, browserInfo, blockedURLPatterns, fetchAbortController, documentInfo } = pageContext;
	const { realm } = params.source;
	const { requestId, url, options: fetchOptions, documentUrl, metaReferrerPolicy } = JSON.parse(params.data.value);
...
async function loadPage(pageContext) {
	...
	const state = { reachedStateIndex: -1, navigation: undefined, navigationErrorText: undefined, pendingRequests: new Set(), loaded: false };
	...
	return new Promise((resolve, reject) => {
		...
		removers.push(listen(pageContext, "browsingContext.navigationStarted", params => {
			if (params.context === context) {
				...
				state.navigationErrorText = undefined;
		...
		removers.push(listen(pageContext, "browsingContext.navigationFailed", params => {
			if (params.context === context && params.navigation === state.navigation) {
				...
				reject(new Error(UNREACHABLE_URL_ERROR_MESSAGE + ": " + params.url +
					(state.navigationErrorText ? " (" + state.navigationErrorText + ")" : "")));
			}
		}));
		...
		removers.push(listen(pageContext, "network.fetchError", params => {
			if (isOwnContext(pageContext, params)) {
				if (params.navigation && params.navigation === state.navigation) {
					state.navigationErrorText = params.errorText;
				}
				state.pendingRequests.delete(params.request.request);
				scheduleNetworkIdleCheck();
			}

...
import { getExecutablePaths, findExecutablePath, copyProfile, spawnBrowser, getDebugPort } from "./browser.js";
...
const { makeTempDir, readTextFile, writeTextFile, errors, remove } = Deno;
const { join } = path;
...
async function launchFirefox(options = {}) {
	const executablePath = options.executablePath || await findExecutablePath(getExecutablePaths(FIREFOX_PATHS));
	if (!executablePath) {
		throw new Error("Firefox was not found in the default locations, use --browser-executable-path to set its location");
	}
...
async function closeFirefox() {
	console.error("DIAG close start at " + Date.now());
	const closedChild = child;
	const closedProfilePath = profilePath;
	child = undefined;
	profilePath = undefined;
	if (closedChild !== undefined) {
		const exited = await Promise.race([closedChild.status.then(() => true), new Promise(resolve => setTimeout(() => resolve(false), 5000))]);
		console.error("DIAG exited before kill: " + exited + " at " + Date.now());
		try {
			closedChild.kill();
		} catch {
			// ignored
		}
		console.error("DIAG killed, awaiting status at " + Date.now());
		await closedChild.status;
		console.error("DIAG status resolved at " + Date.now());
	}
	if (closedProfilePath !== undefined) {
		try {
			await remove(closedProfilePath, { recursive: true });
		} catch (error) {
			console.error("DIAG remove error: " + error.message);
